# LLM Wiki Index

> Auto-maintained by Easy Code. Do not edit manually.

## Sources
- [[source-prd]] — Product requirements document
- [[source-go-gateway-readme]] — Gateway operational docs
- [[source-providers-yaml]] — Provider/model configuration (updated 2026-06-21)
- [[source-env-example]] — Environment variable template (updated 2026-06-21)
- [[source-deploy-yml]] — CI/CD pipeline (GitHub Actions → server)
- [[source-codebase-2026-06-21]] — Full codebase state capture (OpenRouter removed, alias-only exposure)

## Entities
- [[glm-provider]] — Zhipu GLM (primary) (updated 2026-06-21)
- [[mimo-provider]] — Xiaomi MiMo (fallback 1) (updated 2026-06-21)
- [[longcat-provider]] — Meituan LongCat (fallback 2, opus only) (updated 2026-06-21)
- [[openrouter-provider]] — ~~OpenRouter~~ **REMOVED** 2026-06-21
- [[deepv-provider]] — DeepV Server (EasyCode/DeepVCode DeepSeek, 2 models) (updated 2026-08-24)
- [[chatgpt-codex-provider]] — ChatGPT OAuth (proxy required)
- [[server-deployment]] — Production server (8.141.97.21:4001)

## Concepts
- [[overview]] — Project overview and architecture (updated 2026-06-21)
- [[provider-config]] — How to add/modify providers (updated 2026-06-21)
- [[fallback-chains]] — Auto-fallback routing logic (updated 2026-06-21)
- [[model-aliases]] — Haiku/Sonnet/Opus tier naming (updated 2026-06-21)

## Synthesis
- [[mobile-app-performance]] — React Native app perf fixes (2026-06-29): selectors, sort-in-render, nested FlashList, stable keys
